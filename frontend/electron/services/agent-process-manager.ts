...
import { isWindows, COMMAND_TIMEOUT } from '../types/cli.js'
...
export class AgentProcessManager extends EventEmitter {
  private static instance: AgentProcessManager

  // Active processes
  private processes: Map<string, InternalManagedProcess> = new Map()

  // Output subscribers
  private subscribers: Map<string, Set<OutputCallback>> = new Map()

  // Kill timeout for graceful shutdown
  private readonly KILL_TIMEOUT_MS = 10000
   ...
   * Start a CLI process
   */
  async startCLI(config: CLIStartConfig): Promise<string> {
    ...
    const managedProcess: InternalManagedProcess = {
      id: sessionId,
      tool,
  ...
  private setupProcessHandlers(sessionId: string, managedProcess: InternalManagedProcess): void {
  ...
  private handleOutput(sessionId: string, type: 'stdout' | 'stderr', data: string): void {
    const managedProcess = this.processes.get(sessionId)
  ...
  private emitSystemMessage(sessionId: string, message: string): void {
    const outputData: OutputData = {
      sessionId,
      type: 'system',
      data: `[System] ${message}\n`,
  ...
  private detectErrors(output: string): CLIError | null {
    // Rate limit patterns
  ...
  private forceKill(managedProcess: InternalManagedProcess): void {
...
// Internal type with additional fields
interface InternalManagedProcess extends ManagedProcess {
  process: ChildProcess
  outputBuffer: Array<{ type: 'stdout' | 'stderr'; data: string; timestamp: number }>
}
