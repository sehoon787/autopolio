...
export interface BackendManagerConfig {
  ...
  isDev: boolean
  maxRestarts: number
  getConsistentUserDataPath: () => string
  getOrCreateSecretKey: () => string
  getPidFilePath: () => string
}

let config: BackendManagerConfig

export function initBackendManager(cfg: BackendManagerConfig): void {
  config = cfg
}
...
export function savePidFile(pid: number): void {
  try {
...
export async function killExistingBackend(): Promise<void> {
