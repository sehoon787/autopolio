...
let activeLoginTool: string | null = null
...
export interface CLIAuthIPCDeps {
 ...
 */
async function checkClaudeAuthStatus(cliPath: string): Promise<CLIAuthStatus> {
 ...
 */
function checkGeminiAuthStatus(): CLIAuthStatus {
 ...
 */
async function getCLIPath(tool: string): Promise<string | null> {
...
export function registerCLIAuthIPC(deps: CLIAuthIPCDeps): void {
  // ============================================================================
  // cli-auth:status — Quick auth status check (no token consumed)
  // ============================================================================
  ipcMain.handle('cli-auth:status', async (_event, tool: string) => {
    console.log('[IPC] cli-auth:status called for', tool)

    if (tool === 'codex_cli') {
      const cliPath = await getCLIPath('codex_cli')
      if (!cliPath) {
        return { authenticated: false, error: 'Codex CLI not found' } as CLIAuthStatus
      }
    ...
    if (tool === 'claude_code') {
      ...
      if (!cliPath) {
        return { authenticated: false, error: 'Claude Code CLI not found' } as CLIAuthStatus
      }
      return checkClaudeAuthStatus(cliPath)
    }

    return { authenticated: false, error: `Unknown tool: ${tool}` } as CLIAuthStatus
  })

  // ============================================================================
  // cli-auth:login — Start CLI login process
  // ============================================================================
  ipcMain.handle('cli-auth:login', async (_event, tool: string) => {
    ...
    if (!cliPath) {
      return { success: false, tool, error: `${tool} CLI not found` } as CLILoginResult
    }
    ...
    return { success: false, tool, error: `Native login not supported for ${tool}` } as CLILoginResult
  })
  ...
  // ============================================================================
  ipcMain.handle('cli-auth:logout', async (_event, tool: string) => {
 ...
 */
function readCodexAuthFile(): { authenticated: boolean; method?: 'oauth' | 'api_key'; account?: string } {
 ...
 */
async function checkCodexAuthStatus(cliPath: string): Promise<CLIAuthStatus> {
  return new Promise((resolve) => {
    try {
...
/**
 * Common login process: spawn CLI, optionally watch for URL, verify auth on completion.
 * Used by all three CLIs to avoid duplicated boilerplate.
 */
...
function startClaudeLogin(cliPath: string, deps: CLIAuthIPCDeps) {
