 ...
 */
function getClaudeDetectionPaths() {
 ...
 */
function getGeminiDetectionPaths() {
    const homeDir = os.homedir();
    if (isWindows) {
        return [
 ...
 */
async function findInPath(executable) {
    try {
        if (isWindows) {
            // Try 'where' command
 ...
 */
async function validateCLI(cliPath) {
    try {
 ...
 */
async function getLatestClaudeVersion() {
    // Check cache
 ...
 */
async function getLatestGeminiVersion() {
    // Check cache
