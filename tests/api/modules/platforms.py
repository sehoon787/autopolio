...
class PlatformsAPI(BaseAPIModule):
    ...
    def render(self, platform_id: int, user_data: dict) -> httpx.Response:
        """
    ...
    def render_from_db(self, platform_id: int, user_id: int) -> httpx.Response:
        """
        Render platform template with data from database.
        This is a GET endpoint with user_id as query param.
        """
        return self._get(
            f"/platforms/{platform_id}/render-from-db", params={"user_id": user_id}
        )
    ...
    def export_from_db_html(self, platform_id: int, user_id: int) -> httpx.Response:
        """Export platform template as HTML using DB data."""
        return self._post(
            f"/platforms/{platform_id}/export-from-db/html", params={"user_id": user_id}
        )

    def export_from_db_markdown(self, platform_id: int, user_id: int) -> httpx.Response:
        """Export platform template as Markdown using DB data."""
        return self._post(
            f"/platforms/{platform_id}/export-from-db/md", params={"user_id": user_id}
        )

    def export_from_db_docx(self, platform_id: int, user_id: int) -> httpx.Response:
        """Export platform template as Word doc using DB data."""
        return self._post(
            f"/platforms/{platform_id}/export-from-db/docx", params={"user_id": user_id}
        )

    def export_html(
        self, platform_id: int, render_data: Optional[dict] = None
    ) -> httpx.Response:
    ...
    def export_markdown(
        ...
        data = render_data or _SAMPLE_RENDER_DATA
        return self._post(
            f"/platforms/{platform_id}/export/md", json={"data": data, "format": "md"}
        )
