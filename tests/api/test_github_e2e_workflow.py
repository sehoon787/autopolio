...
class TestElectronWorkflow:
    ...
    def test_electron_save_token_flow(
        self, api_client, test_user, gh_token, gh_username
    ):
        """
        ...
        print("[Electron] GitHub connected and valid")
    ...
    def test_electron_import_only(self, api_client, test_user, gh_token):
        ...
        existing_response = api_client.get(
            "/knowledge/projects", params={"user_id": actual_user_id}
        )
        existing_urls = set()
        ...
        for repo in repos[:50]:
            url = (
                (repo.get("html_url") or repo.get("clone_url", ""))
                .lower()
                .replace(".git", "")
            )
        ...
        assert import_response.status_code == 200, (
            f"Import failed: {import_response.text}"
        )
        ...
        project_response = api_client.get(
            f"/knowledge/projects/{project_id}", params={"user_id": actual_user_id}
        )
        ...
        print("[Electron Import] Cleaned up")
    ...
    def test_electron_import_and_analyze_flow(self, api_client, test_user, gh_token):
        ...
        try:
            ...
            assert analyze_response.status_code == 200, (
                f"Analysis start failed: {analyze_response.text}"
            )
            ...
            result = wait_for_analysis_completion(
                api_client, project_id, actual_user_id, max_wait=180, poll_interval=5
            )
            ...
            if result["completed"]:
                ...
                print("[Electron] Analysis verification:")
                print(f"  - Commits: {analysis.get('total_commits', 'N/A')}")
                print(
                    f"  - Technologies: {analysis.get('detected_technologies', [])[:5]}"
                )
                print(f"  - Key tasks: {len(analysis.get('key_tasks', []))} items")
                print(
                    f"  - Implementation details: {len(analysis.get('implementation_details', []))} sections"
                )
                print(
                    f"  - AI Summary: {'Yes' if analysis.get('ai_summary') else 'No'}"
                )
                ...
                assert verification["passed"], (
                    f"Analysis missing required fields: {verification['missing_fields']}"
                )
            else:
...
class TestWebWorkflow:
    ...
    def test_web_import_only(self, api_client, github_user):
        ...
        # Get existing
        existing_response = api_client.get(
            "/knowledge/projects", params={"user_id": user_id}
        )
        existing_urls = set()
        if existing_response.status_code == 200:
            for p in existing_response.json().get("projects", []):
    ...
    def test_web_import_and_analyze_flow(self, api_client, github_user):
        ...
        try:
            ...
            result = wait_for_analysis_completion(
                api_client, project_id, user_id, max_wait=180, poll_interval=5
            )
            ...
            if result["completed"]:
                ...
                print("[Web] Analysis verification:")
                print(f"  - Commits: {analysis.get('total_commits', 'N/A')}")
                print(
                    f"  - Technologies: {analysis.get('detected_technologies', [])[:5]}"
                )
                ...
                print(
                    f"  - Detailed achievements: {len(analysis.get('detailed_achievements', {}))} categories"
                )
                print(
                    f"  - AI Summary: {'Yes' if analysis.get('ai_summary') else 'No'}"
                )
...
class TestAnalysisContentVerification:
    ...
    def test_analysis_has_implementation_details(self, api_client, github_user):
        ...
        try:
            ...
            result = wait_for_analysis_completion(
                api_client, project_id, user_id, max_wait=180
            )
            ...
            if result["completed"]:
                ...
                if impl_details:
                    assert isinstance(impl_details, list), (
                        "implementation_details should be a list"
                    )
                    if len(impl_details) > 0:
                        first_detail = impl_details[0]
                        assert "title" in first_detail, (
                            "Each detail should have a title"
                        )
                        assert "items" in first_detail, "Each detail should have items"
                        print(
                            f"[Verify] First implementation detail: {first_detail['title']}"
                        )
                else:
    ...
    def test_compare_cli_vs_api_analysis(self, api_client, github_user):
        ...
        if import_response.status_code == 200:
            ...
            try:
                ...
                if result["completed"]:
                    ...
                    print("[Compare] API mode completed")
                else:
        ...
        if "api" in results:
            ...
            if api_result.get("passed"):
                ...
                print(
                    f"  API mode: FAILED - {api_result.get('missing_fields', api_result.get('error'))}"
                )
