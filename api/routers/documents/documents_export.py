...
async def get_export_preview(
    ...
    report_type: Literal["detailed", "final", "summary"] = Query(
        "summary", description="Report type"
    ),
    include_code_stats: bool = Query(
        False, description="Include code statistics (lines added/deleted, commits)"
    ),
    ...
    try:
        preview = await export_service.get_export_preview(
            user_id, report_type, include_code_stats
        )
        return preview
...
async def export_to_markdown(
    ...
    export_service = ExportService(db, language=language)
    try:
        file_path, content = await export_service.export_to_markdown(
            user_id, report_type, include_code_stats
        )
...
async def export_to_docx(
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to generate Word document: {str(e)}"
        )
...
async def get_single_project_export_preview(
    ...
    export_service = ExportService(db, language=language)
    try:
        preview = await export_service.get_single_project_preview(
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Single project export preview failed: {str(e)}"
        )
...
async def export_single_project_to_markdown(
    ...
    export_service = ExportService(db, language=language)
    try:
        file_path, content = await export_service.export_single_project_to_markdown(
            project_id, report_type, include_code_stats
        )
        filename = os.path.basename(file_path)
        return {
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Single project markdown export failed: {str(e)}"
        )
...
async def export_single_project_to_docx(
    ...
    export_service = ExportService(db, language=language)
    try:
        file_path = await export_service.export_single_project_to_docx(
            project_id, report_type, include_code_stats
        )
        filename = os.path.basename(file_path)
        return {
