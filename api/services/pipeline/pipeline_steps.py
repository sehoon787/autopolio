...
async def generate_single_summary(
    semaphore: asyncio.Semaphore, llm_service, project: Project, summary_style: str
) -> Dict[str, Any]:
        ...
        try:
            ...
            if project.repo_analysis:
                ...
                project_data["commit_summary"] = (
                    project.repo_analysis.commit_messages_summary
                )

            # Generate summary
            summary_result = await llm_service.generate_project_summary(
                project_data, summary_style
            )
            ...
            logger.warning(
                "Failed to generate summary for project %s: %s", project.id, e
            )
            return {
                "project_id": project.id,
                "project": project,
    ...
    for analysis in analyses:
        ...
        if categories:
            results["architectures"].append(
                {"project_id": analysis.project_id, "commit_distribution": categories}
            )
...
async def step_achievement_detection(
    ...
    for project in projects:
        ...
        detected_achievements, stats = await achievement_service.detect_all(
            project_data=project_data, commit_messages=commit_messages, use_llm=False
        )
    ...
    try:
        ...
        for project in projects:
            ...
            if project.repo_analysis and project.repo_analysis.ai_summary:
                ...
                logger.info(
                    "Copied ai_summary from RepoAnalysis for project %d", project.id
                )
            else:
        ...
        if projects_needing_llm:
            logger.info(
                "Generating LLM summaries for %d projects without analysis",
                len(projects_needing_llm),
            )
            ...
            if cli_mode:
                ...
                logger.info(
                    "Using API mode: provider=%s, model=%s", provider, user_model
                )
                llm_service = LLMService(provider, model=user_model)

            # Get user's summary style preference
            summary_style = (
                getattr(user, "default_summary_style", "professional") or "professional"
            )

            # Parallel summarization
            semaphore = asyncio.Semaphore(MAX_CONCURRENT_LLM_SUMMARY)
            tasks = [
                generate_single_summary(semaphore, llm_service, project, summary_style)
            ...
            for result in summary_results:
                ...
                results["summaries"].append(
                    {"project_id": project.id, "summary": summary, "source": "llm"}
                )
