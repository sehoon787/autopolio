...
def build_projects(
    projects: List[Project], companies: List[Company]
    ...
    for idx, project in enumerate(projects, 1):
        ...
        if ra:
            ...
            if effective_key_tasks:
                key_tasks = (
                    effective_key_tasks if isinstance(effective_key_tasks, list) else []
                )
            ...
            if effective_impl_details:
                impl_details = (
                    effective_impl_details
                    if isinstance(effective_impl_details, list)
                    else []
                )
            ...
            if effective_detailed_achievements:
                achievements_data = (
                    ...
                    if isinstance(effective_detailed_achievements, dict)
                    else {}
                )
        ...
        achievements_basic, achievements_basic_list = _build_achievements_basic(
            project_achievements
        )
        achievements_summary, achievements_summary_list, achievements_grouped = (
            _build_achievements_summary(
                detailed_achievements, project_achievements, achievements_basic
            )
        )
        achievements_detailed, achievements_detailed_list = (
            _build_achievements_detailed(detailed_achievements, project_achievements)
        )
        ...
        proj = {
            ...
            "start_date": project.start_date.strftime("%Y.%m")
            ...
            "achievements_list": achievements_summary_list
            or achievements_basic_list,  # For template iteration
    ...
    if detailed_achievements:
        ...
        for ach in project_achievements:
            ...
            if cat not in cat_map:
                cat_map[cat] = []
            cat_map[cat].append(
                {"title": ach.get("metric_value", ach.get("title", ""))}
            )
...
def _build_per_repo_analyses(
    project, code_stat_keywords: List[str]
) -> List[Dict[str, Any]]:
