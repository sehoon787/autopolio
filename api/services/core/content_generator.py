...
JSON만 반환하세요."""
    ...
    return await _call_and_parse_json(
        ...
        prompt,
        system_prompt,
        LLM_MAX_TOKENS.CONTENT_GENERATION,
        fallback=[],
    )
...
JSON만 반환하세요."""
    ...
    return await _call_and_parse_json(
        ...
        prompt,
        system_prompt,
        LLM_MAX_TOKENS.CONTENT_GENERATION_MID,
        fallback=[],
    )
        ...
        try:
            for key, gen_fn, fallback in gen_tasks:
                try:
                    ...
                    logger.info(
                        "[DetailedContent] %s: %d items, %d tokens", key, count, tokens
                    )
                except Exception as e:
                    logger.error(
                        "[DetailedContent] %s failed: %s: %s", key, type(e).__name__, e
                    )
                    result[key] = fallback
        finally:
            llm.exit_batch_mode()
