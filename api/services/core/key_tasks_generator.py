...
async def generate_key_tasks(
    ...
    try:
        if llm_service is None:
            if cli_mode:
                logger.info(
                    "[KeyTasks] Using CLI mode: %s, model: %s", cli_mode, cli_model
                )
        ...
        logger.debug(
            "[KeyTasks] LLM service type: %s, language: %s",
        ...
        if code_contributions and code_contributions.get("contributions"):
            ...
            if code_snippets:
                ...
                logger.info(
                    "[KeyTasks] Including %d code snippets in context",
                    len(code_snippets[:5]),
                )
