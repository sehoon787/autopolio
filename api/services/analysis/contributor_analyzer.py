...
AI_TOOL_PATTERNS = {
    ...
    "Gemini": [
        ...
        re.compile(r"Co-Authored-By:.*Google", re.IGNORECASE),
    ],
    "ChatGPT": [
        re.compile(r"Co-Authored-By:.*ChatGPT", re.IGNORECASE),
        re.compile(r"Co-Authored-By:.*OpenAI", re.IGNORECASE),
    ],
    "Windsurf": [
        re.compile(r"Co-Authored-By:.*Windsurf", re.IGNORECASE),
        re.compile(r"Co-Authored-By:.*Codeium", re.IGNORECASE),
    ],
}


def detect_ai_tools(commit_messages: List[str]) -> List[Dict[str, Any]]:
    """Detect AI coding tools from commit messages (Co-Authored-By patterns).

    Args:
        commit_messages: List of full commit messages (including body with Co-Authored-By)

    Returns:
        List of dicts: [{"tool": "Claude Code", "evidence": "Co-Authored-By", "count": 42}]
    """
    ...
    for message in commit_messages:
        if not message:
            continue
        for tool_name, patterns in AI_TOOL_PATTERNS.items():
            for pattern in patterns:
                if pattern.search(message):
                    tool_counts[tool_name] = tool_counts.get(tool_name, 0) + 1
                    break  # Only count once per tool per message

    return [
        {"tool": tool, "evidence": "Co-Authored-By", "count": count}
