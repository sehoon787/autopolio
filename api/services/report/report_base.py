...
class ReportBaseService:
    ...
    def _get_effective_implementation_details(
        self, analysis: Any, edits: Any
    ) -> List[str]:
        ...
        return (
            ...
            if analysis and analysis.implementation_details
            else []
        )
    ...
    def _get_effective_detailed_achievements(self, analysis: Any, edits: Any) -> Dict:
        ...
        return (
            analysis.detailed_achievements
            if analysis and analysis.detailed_achievements
            else {}
        )
    ...
    async def _get_user_data(self, user_id: int) -> Dict[str, Any]:
        ...
        return {"user": user, "companies": companies, "projects": projects}

    async def _get_analyzed_projects(
        self, user_id: int
            ...
            if has_multi_repo:
                for a in analyses:
                    ...
                    if a.project_repository:
                        raw = a.project_repository.label or (
                            a.git_url.split("/")[-1] if a.git_url else ""
                        )
                    ...
                    a_edits_result = await self.db.execute(
                        select(RepoAnalysisEdits).where(
                            RepoAnalysisEdits.repo_analysis_id == a.id
                        )
                    )
