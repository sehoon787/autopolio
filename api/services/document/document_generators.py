...
async def generate_pdf(content: str, file_path: Path) -> int:
    ...
    styles.add(
        ParagraphStyle(
            name="CustomBody", fontName=default_font, fontSize=11, leading=14
        )
    )
    styles.add(
        ParagraphStyle(
    ...
    for line in lines:
        ...
        if line.startswith("# "):
            elements.append(
                Paragraph(line[2:].replace("**", ""), styles["CustomHeading1"])
            )
        elif line.startswith("## "):
            elements.append(
                Paragraph(line[3:].replace("**", ""), styles["CustomHeading2"])
            )
        elif line.startswith("### "):
            elements.append(
                Paragraph(f"<b>{line[4:].replace('**', '')}</b>", styles["CustomBody"])
            )
        elif line.startswith("- "):
