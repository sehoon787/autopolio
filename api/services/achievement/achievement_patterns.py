...
ACHIEVEMENT_PATTERNS: Dict[str, List[Tuple[str, str, str]]] = {
    "performance": [
        # Korean patterns
        (
            r"(\d+)\s*%\s*(성능|속도).*(향상|개선|증가|up)",
            "성능 향상",
            "{value}% 성능 향상",
        ),
        (
            r"(\d+)\s*배\s*(성능|속도).*(향상|개선|증가)",
            "성능 향상",
            "{value}배 성능 향상",
        ),
        (
            r"(성능|속도).*(향상|개선|증가).*(\d+)\s*%",
            "성능 향상",
            "{value}% 성능 향상",
        ),
        (
            r"(성능|속도).*(향상|개선|증가).*(\d+)\s*배",
            "성능 향상",
            "{value}배 성능 향상",
        ),
        # English patterns
        (
            r"(\d+)\s*%\s*(performance|speed)\s*(improve|increase|boost)",
            "성능 향상",
            "{value}% 성능 향상",
        ),
    ...
    "efficiency": [
        (
            r"(\d+)\s*%\s*(생산성|효율).*(향상|개선|증가)",
            "생산성 향상",
            "{value}% 생산성 향상",
        ),
        (
            r"(\d+)\s*배\s*(생산성|효율).*(향상|개선|증가)",
            "생산성 향상",
            "{value}배 생산성 향상",
        ),
        (
            r"(시간|작업).*(단축|절감|감소).*(\d+)\s*%",
            "작업 시간 단축",
            "{value}% 작업 시간 단축",
        ),
        (
            r"(\d+)\s*%\s*(time|effort).*(reduce|save)",
            "작업 시간 단축",
            "{value}% 작업 시간 단축",
        ),
    ],
    "reduction": [
        ...
        (
            r"(메모리|memory).*(감소|절감|reduce).*(\d+)\s*%",
            "메모리 사용량 절감",
            "{value}% 메모리 절감",
        ),
        (
            r"(\d+)\s*%\s*(reduce|decrease).*(error|bug)",
            "오류 감소",
            "{value}% 오류 감소",
        ),
    ],
    ...
    "scale": [
        (
            r"(\d+)\s*만?\s*(명|사용자|user|customer).*(달성|도달|확보)",
            "사용자 수",
            "{value}명 사용자 달성",
        ),
    ...
    "cost": [
        (
            r"(비용|cost).*(절감|reduce|save).*(\d+)\s*%",
            "비용 절감",
            "{value}% 비용 절감",
        ),
        (
            r"(\d+)\s*%\s*(비용|cost).*(절감|reduce|save)",
            "비용 절감",
            "{value}% 비용 절감",
        ),
        (r"(\d+)\s*원.*(절감|절약|감소)", "비용 절감", "{value}원 비용 절감"),
        (
            r"(서버|인프라).*(비용|cost).*(\d+)\s*%\s*(절감|감소)",
            "인프라 비용 절감",
            "{value}% 인프라 비용 절감",
        ),
    ],
    "quality": [
        (
            r"(코드\s*품질|code\s*quality).*(\d+)\s*%\s*(향상|개선)",
            "코드 품질 향상",
            "{value}% 코드 품질 향상",
        ),
        (
            r"(테스트\s*커버리지|test\s*coverage).*(\d+)\s*%",
            "테스트 커버리지",
            "테스트 커버리지 {value}%",
        ),
        (
            r"(커버리지|coverage).*(\d+)\s*%\s*(달성|향상)",
            "테스트 커버리지",
            "커버리지 {value}% 달성",
        ),
    ...
    "delivery": [
        (
            r"(배포\s*주기|deploy\s*cycle).*(\d+)\s*%\s*(단축|개선)",
            "배포 주기 단축",
            "{value}% 배포 주기 단축",
        ),
        (
            r"(개발\s*시간|development\s*time).*(\d+)\s*%\s*(단축|절감)",
            "개발 시간 단축",
            "{value}% 개발 시간 단축",
        ),
        (r"(\d+)\s*회\/?(주|월|일).*(배포|deploy)", "배포 빈도", "{value}회 배포"),
        (
            r"(리드\s*타임|lead\s*time).*(\d+)\s*%\s*(단축|감소)",
            "리드 타임 단축",
            "{value}% 리드 타임 단축",
        ),
    ],
    "user": [
        (
            r"(사용자\s*만족도|user\s*satisfaction).*(\d+)\s*%",
            "사용자 만족도",
            "사용자 만족도 {value}%",
        ),
        (r"(NPS|nps).*(\d+)", "NPS", "NPS {value}"),
        (
            r"(이탈률|churn).*(\d+)\s*%\s*(감소|줄)",
            "이탈률 감소",
            "{value}% 이탈률 감소",
        ),
        (
            r"(전환율|conversion).*(\d+)\s*%\s*(증가|향상)",
            "전환율 향상",
            "{value}% 전환율 향상",
        ),
...
COMMIT_ACHIEVEMENT_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    (
        r"(버그|bug|오류|에러)\s*#?(\d+)\s*(수정|fix|해결)",
        "버그 수정",
        "#{value} 버그 수정",
    ),
    ...
    (
        r"(\d+)\s*개?\s*(기능|feature).*(추가|구현|개발)",
        "기능 추가",
        "{value}개 기능 추가",
    ),
    (
        r"(기능|feature)\s*(\d+)\s*개?\s*(추가|구현|개발)",
        "기능 추가",
        "{value}개 기능 추가",
    ),
    (
        r"(\d+)\s*개?\s*(파일|모듈).*(리팩토링|refactor|정리)",
        "리팩토링",
        "{value}개 파일 리팩토링",
    ),
    ...
    (
        r"(응답\s*시간|response\s*time).*(\d+)\s*%\s*(단축|개선|감소)",
        "응답 시간 개선",
        "{value}% 응답 시간 개선",
    ),
    (r"(\d+)\s*ms.*(\d+)\s*ms.*(단축|개선)", "응답 시간 개선", "응답 시간 개선"),
    (
        r"(처리량|throughput).*(\d+)\s*(배|%)\s*(증가|향상)",
        "처리량 향상",
        "{value}배 처리량 향상",
    ),
    # Korean patterns - 배포/자동화
    (
        r"(배포\s*시간|deploy).*(\d+)\s*%\s*(단축|개선)",
        "배포 시간 단축",
        "{value}% 배포 시간 단축",
    ),
